		...
		for (int i = 0; i < grafo.getTamaño(); i++) {
			for (int j = 0; j < grafo.getTamaño(); j++) {
				if (matrizCostos[i][j] != grafo.INFINITO) {
					System.out.println("El costo de ir de " + grafo.getListaNodos().get(i).getNickname() + " a "
							+ grafo.getListaNodos().get(j).getNickname() + " es de " + matrizCostos[i][j] + "m");
				} else if (matrizCostos[i][j] == 0) {
					System.out.println("Ya se esta en " + grafo.getListaNodos().get(i).getNickname());
				} else {
					System.out.println("No hay camino de " + grafo.getListaNodos().get(i).getNickname() + " a "
