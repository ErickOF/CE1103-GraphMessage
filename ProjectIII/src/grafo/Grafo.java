...
public class Grafo {
	...
	public void agregarCliente(Cliente cliente) {
		// Recorrer para limpiar los enlaces
		for (Nodo nodo : listaNodo) {
			nodo.setListaNodoAdyacente();
		} // Situar el nodo
		Coordenadas coordenadas = new Coordenadas(100000, 100000, random.nextInt(550), random.nextInt(550));
		// Crear el nodo
		Nodo nCliente = new Nodo(cliente.getNombre(), coordenadas);
		// Dar el MacAdress del dispositivo
		nCliente.setMacAdress(cliente.getMacAddress());
		// Configurar el circulo que representa el nodo
		nCliente.getCirculo().setDiametro(12);
		nCliente.getCirculo().setEtiqueta(nCliente.getNickname() + "");
		nCliente.getCirculo().setColor(Color.RED);
		nCliente.setCliente(true);
		// Agregar el nuevo nodo
		listaNodo.add(nCliente);
		// Aumentar tamaño del grafo
		tamaño++;
		// Volver a enlazar el grafo
		enlazarGrafo();
	}
